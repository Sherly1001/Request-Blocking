$(document).ready(function () {
  ...
  $(".block-urls").on("click", ".tg", function () {
    !(function ($tg) {
      chrome.storage.sync.get("block-urls", function (o) {
        ...
        chrome.storage.sync.set({ "block-urls": o["block-urls"] });
        chrome.runtime.sendMessage({ urls: o["block-urls"] });
      });
    })($(this));
  });
  ...
  chrome.storage.sync.get("block-urls", function (o) {
    for (let url in o["block-urls"]) {
      $(".block-urls")
        .append(
          `<div><input type="text" value="${url}" readonly><div class="bt tg"><i class="fas fa-toggle-${
            o["block-urls"][url] ? "on" : "off"
          }"></i></div><div class="bt rm"><i class="fas fa-trash-alt"></i></div></div>`
        )
        .show();
    }
  });
          ...
          $(".block-urls")
            .append(
              '<div><input type="text" value="' +
                url +
                '" readonly><div class="bt tg"><i class="fas fa-toggle-on"></i></div><div class="bt rm"><i class="fas fa-trash-alt"></i></div></div>'
            )
            .show();
